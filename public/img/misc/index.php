...
try {
	$db = new PDO('mysql:host=localhost;dbname=csgo', 'root', 'root', array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
...
if ($MTN) {
	if ($user && $user['steamid'] == "76561197963152633" || $user && $user['steamid'] == "76561197963152633" || $user && $user['steamid'] == "76561197963152633") {
		$MTN = false;
	}
else 
...
switch ($_GET['page']) {
	...
	case 'redeem':
		...
		$out = curl('http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=76EA12089450A0A6F64892830FAACB8F&steamid='.$user['steamid'].'&format=json');
        ...
        try
        {
            ...
            if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					$url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=76EA12089450A0A6F64892830FAACB8F&steamids=$matches[1]";
...
function getUserSteamAvatar($steamid){
    $link = file_get_contents('http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=76EA12089450A0A6F64892830FAACB8F&steamids='.$steamid.'&format=json');
...
function getUserSteamNicknameFromDB($steamid){
	$db = new PDO('mysql:host=localhost;dbname=csgoturbo', 'root', 'root', array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
