...
$(function() {
    ...
    window.roulette = ({
        ...
        updateTimer: function() {
            if (this.timeLeft > 0) {
                $('.rolling').fadeIn(200);
                $('.rolling-inner').html('<div>' + (this.timeLeft / 100).toFixed(2).replace(".",":") + '</div>Прокрутка начнется через');
            } else {
                $('.rolling').fadeOut(200);
            }
        },
    ...
    socket.on('roulette new round', function(time, hash) {
        ...
        $('.roulette-info').text('ХЭШ РАУНДА: ' + hash);
    });
    ...
    socket.on('roulette round', function(time, bets, hash) {
        ...
        $('.roulette-info').html('ХЭШ РАУНДА: ' + hash);
    });
