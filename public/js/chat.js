        ...
        displayMessage: function(item, bypass) {
            ...
            if (item.hasOwnProperty('profile')) {
                $('<div class="chat-message" data-uniqueid="' + item.uniqueID + '" data-id="' + item.profile.steamid + '" data-username="' + item.profile.username + '"> ' +
                    ...
                    '<div class="message"> ' +
                    '<div class="nickname ' + item.profile.rank + '"><a target="_blank" href="//vk.com/id' + item.profile.steamid + '">' + item.profile.username + '</a>' + '<span style="opacity: 0.5;font-size: 12px;color: #e1e7ee;float:right;"> ' + chat.timeConvert(item.time) + '</span>' + '</div> ' +
        ...
        items: function(e) {
            ...
            return [
                ...
                {type: 'item', text: locale.visitProfile, click: function() {
                    var win = window.open('https://vk.com/id' + user.data('id'), '_blank');
                    win.focus();
                }}
            ]
        }
