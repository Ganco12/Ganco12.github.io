$(function() {
    ...
    window.coinflip = ({
        ...
        _generateBetFromData: function(data) {
            ...
            if(data.status === 'open') text += '<button data-hash="' + data.hash + '" style="outline: 0; border: 0; height: 25px; width: 7%; margin-top: 1%; font-size: 18px; line-height: 10px; color: #161616; background: linear-gradient(90deg,#00a25b 0,#6ae855); font-weight: 700; cursor: pointer; text-transform: uppercase; font-family: Titillium,sans-serif; border-radius: 5px; font-weight: 700; transition: all .3s; border: 0px solid #00ffe4;" class="btn-join" onclick="socket.emit(\'coinflip join\', \'' + data.hash + '\');">ИГРАТЬ</button>';
            if(!data.opponent) text += '<div class="amount" data-worth="' + data.amount + '">Ставка: ' + data.amount + '</div>';
            else {
                text += '<div class="amount" data-worth="' + data.amount + '">' + '<div class="flip-container" style="width: 40px;height: 40px;position: relative;"><div class="flip" style="position: absolute; left: 50%; top: 50%; padding: 0px; margin: 0px; transform: translate(-50%, -50%);">' + data.left + '</div></div><br/><div class="amount" data-worth="' + data.amount + '">Ставка: ' + data.amount + '</div>' + '</div>';
            }
            if(data.opponent) { 
        ...
        countDown: function(hash,time){
            ...
            var countDownInterval = setInterval(function() {
                ...
                if (time <= 3){
                    $('.'+hash+' .amount .flip').css('color','orange');
                }
                if (time <= 1) {
